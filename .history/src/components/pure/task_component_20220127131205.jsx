...
const TaskComponent = ({task, completed}) => {
    ...
    function taskCompletedIcon(){
        if(task.completed){
            return(<i onClick={() => completed(task)} className='bi bi-journal-check' style={{color: 'green', fontWeight:'bold'}}></i>);
        }else{
            return(<i onClick={() => completed(task)} className='bi bi-journal-x' style={{color: 'blue', fontWeight:'bold'}}></i>);
        }
    }
